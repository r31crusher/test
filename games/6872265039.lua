...
return function(section)
    ...
    RunSvc.Heartbeat:Connect(function()
        ...
        pcall(function()
            bd.Blink.item_action.place_block.invoke({position = pos, block_type = blockTool.Name, extra = {}})
        end)
    end)
    ...
    RunSvc.Heartbeat:Connect(function()
        ...
        for _, p in Players:GetPlayers() do
            ...
            if eh and (eh.Position - _bedPart.Position).Magnitude < _bedDefRange then
                for dx = -2, 2, 2 do for dz = -2, 2, 2 do for dy = 0, 2 do
                    local pos = Vector3.new(
                        math.round(_bedPart.Position.X + dx),
                        math.round(_bedPart.Position.Y + dy),
                        math.round(_bedPart.Position.Z + dz))
                    pcall(function()
                        bd.Blink.item_action.place_block.invoke({position = pos, block_type = "OakPlanks", extra = {}})
                    end)
                end end end
            end
        end
                        ...
                        if hrp then
                            for _, obj in workspace:GetDescendants() do
                                ...
                                if nameLower:find("chest") and not nameLower:find("chestplate") then
                                    ...
                                    if part and part:IsA("BasePart") and (part.Position - hrp.Position).Magnitude < _chestRange then
                                        pcall(function() bd.Blink.item_action.open_chest.fire({chest_id = obj:GetFullName()}) end)
                                    end
                                end
                            end
                        end
        ...
        for _, item in player.Backpack:GetChildren() do
            ...
            for _, slot in _armorSlots do
                if nameLower:find(slot) then
                    pcall(function()
                        bd.Blink.player_state.equip_item.invoke({slot = slot, item = item.Name})
                    end)
                    break
                end
            end
    ...
    player.CharacterAdded:Connect(function()
        task.wait(1); if getgenv()._bdAutoArmor then _tryEquipArmor() end
    end)
    ...
    local function _removeESP(p) if _espBills[p] then _espBills[p].bb:Destroy(); _espBills[p] = nil end end
    ...
    elements:Toggle("ESP", section, function(v)
        getgenv()._bdESP = v; if v then _startESP() else _stopESP() end
    end)

    -- ── Unload ────────────────────────────────────────────────────────────────
    section.AncestorRemoving:Connect(function()
        getgenv()._bdKillaura    = false; getgenv()._bdReach       = false
        getgenv()._bdAutoClick   = false; getgenv()._bdTriggerbot  = false
        getgenv()._bdVelocity    = false; getgenv()._bdCrits       = false
        getgenv()._bdHitboxes    = false; getgenv()._bdProjAim     = false
        getgenv()._bdProjAura    = false; getgenv()._bdAutoBuy     = false
        getgenv()._bdNoFall      = false; getgenv()._bdScaffold    = false
        getgenv()._bdBedDefender = false; getgenv()._bdChestFarm   = false
        getgenv()._bdAutoArmor   = false; getgenv()._bdESP         = false
        if _velOrig and _velConn then pcall(hookfunction, _velConn.Function, _velOrig) end
        if _critsOrig and _loaded then pcall(function() hookfunction(bd.Blink.item_action.attack_entity.fire, _critsOrig) end) end
        if _paOld and _loaded then pcall(function() hookfunction(debug.getupvalue(bd.BowClient.Start, 11), _paOld) end) end
        if _nofallOrig and _loaded then pcall(function() hookfunction(bd.Blink.player_state.take_fall_damage.fire, _nofallOrig) end) end
        if _reachOrig ~= nil and _loaded then pcall(function() rawset(bd.CombatConsts, "REACH_IN_STUDS", _reachOrig) end) end
